...
var db = { units:"lb", days:{}, exerciseLib:{}, plans:[], runningPlanId:null };
var tab = "log";
var cursor = todayKey();
var openExercise = null;   // history detail
var historyType = "all";  // history type filter
var libType = "push";     // exercise-library panel filter
var libEdit = null;       // name currently being renamed in library panel
var planDraft = null;     // {id,name,items,editing} while the plan editor is open
var piType = "push", piName = "", piSets = 3;  // plan-item builder fields
...
function suggestNamesByType(type){
  var seen={}, out=[];
  (db.exerciseLib[type]||[]).forEach(function(n){
    var k=n.toLowerCase(); if(!seen[k]){ seen[k]=1; out.push(n); }
  });
  exerciseNamesByType(type).forEach(function(n){
    var k=n.toLowerCase(); if(!seen[k]){ seen[k]=1; out.push(n); }
  });
  return out;
}
function allLibNames(){
  var seen={}, out=[];
  TYPES.forEach(function(t){
    (db.exerciseLib[t.key]||[]).forEach(function(n){
      var k=n.toLowerCase(); if(!seen[k]){ seen[k]=1; out.push(n); }
    });
  });
  return out;
}
function libAdd(type, name){
  name = (name||"").trim(); if(!name) return;
  db.exerciseLib[type] = db.exerciseLib[type] || [];
  var lc = name.toLowerCase();
  if(db.exerciseLib[type].some(function(n){ return n.toLowerCase()===lc; })) return;
  db.exerciseLib[type].push(name);
  save();
}
function libDelete(type, name){
  db.exerciseLib[type] = (db.exerciseLib[type]||[]).filter(function(n){ return n!==name; });
  save();
}
function libRename(type, oldName, newName){
  newName = (newName||"").trim(); if(!newName) return;
  db.exerciseLib[type] = db.exerciseLib[type] || [];
  var arr = db.exerciseLib[type];
...
function weightBreakdown(ss){
  // per weight used, the most reps ever done at that weight
  var map = {};
  ss.forEach(function(s){ s.sets.forEach(function(x){
    var k = trim(x.weight);
    if(!map[k] || x.reps>map[k].reps) map[k] = {weight:x.weight, reps:x.reps};
  }); });
...
function renderLog(){
  ...
  var html = "";
  var runPlan = db.runningPlanId ? planById(db.runningPlanId) : null;
  if(db.runningPlanId && !runPlan){ db.runningPlanId=null; save(); }
  if(runPlan) html += renderPlanBar(runPlan);
...
function renderPlanBar(plan){
  var h = '<div class="planbar"><div class="pbhead"><span class="t">Plan · '+esc(plan.name)+'</span>'+
    '<button class="ghostbtn" data-endplan="1">End</button></div><div class="pitems">';
  plan.items.forEach(function(item, i){
    var entry = dayEntries(cursor).filter(function(e){ return e.name.toLowerCase()===item.name.toLowerCase(); })[0];
    var done = entry ? entry.sets.length : 0;
    var complete = done>=item.sets;
    h += '<button class="pirow'+(complete?' done':'')+'" data-planitem="'+i+'">'+
      '<span class="tbadge" style="color:'+typeColor(item.type)+'">'+esc(typeLabel(item.type))+'</span>'+
      '<span class="nm">'+esc(item.name)+'</span>'+
      '<span class="prog">'+done+'/'+item.sets+'</span></button>';
  });
  h += '</div></div>';
  return h;
}
...
function renderPlans(){
  dayTitle.textContent = "Plans";
  var html = '<button class="ghostbtn" id="manageLib" style="margin-bottom:16px;width:100%">Manage exercise library</button>';
  html += '<div class="qhead" style="margin:4px 0 10px;border-bottom:1px solid #262a31;padding-bottom:8px;font-family:var(--mono);font-size:10px;letter-spacing:.14em;text-transform:uppercase;color:var(--steel)">Workout plans</div>';
  if(!db.plans.length){
    html += '<div class="empty"><div class="big">No plans yet</div>'+
            '<p>Build a plan from exercises and target sets, then work through it in any order.</p></div>';
  } else {
    db.plans.forEach(function(p){
      var totalSets = p.items.reduce(function(t,i){ return t+i.sets; },0);
      var running = db.runningPlanId===p.id;
      html += '<div class="planrow'+(running?' active':'')+'">'+
        '<button class="planinfo" data-editplan="'+p.id+'"><span class="nm">'+esc(p.name)+'</span>'+
        '<div class="sub">'+p.items.length+' exercise'+(p.items.length>1?"s":"")+' · '+totalSets+' sets'+(running?' · running':'')+'</div></button>'+
        '<button class="ghostbtn" data-startplan="'+p.id+'">'+(running?"Resume":"Start")+'</button></div>';
    });
  }
  html += '<button class="add" id="newPlan" style="margin-top:14px">+ New plan</button>';
  view.innerHTML = html;
}
...
function renderExercise(){
  ...
  var wb = weightBreakdown(ss);
  if(wb.length){
    html += '<div class="wtable"><div class="cap">By weight · max reps &amp; projected 1RM</div>';
    wb.forEach(function(r){
      var rm = e1rm({weight:r.weight, reps:r.reps});
      var isBest = Math.abs(rm-bestRm)<0.05;
      html += '<div class="wrow'+(isBest?' best':'')+'"><span class="w">'+trim(r.weight)+' '+db.units+'</span>'+
              '<span class="r">'+r.reps+' rep'+(r.reps>1?'s':'')+' max</span>'+
              '<span class="erm">'+trim(rm)+' e1RM</span></div>';
    });
    html += '</div>';
  }
...
function refreshDatalist(){
  var seen={}, out=[];
  exerciseNames().concat(allLibNames()).forEach(function(n){
    var k=n.toLowerCase(); if(!seen[k]){ seen[k]=1; out.push(n); }
  });
  document.getElementById("exlist").innerHTML =
    out.map(function(n){ return '<option value="'+esc(n)+'">'; }).join("");
}

/* ---------------- entry panel ---------------- */
var draft = null;   // {id, name, sets:[], reps, weight, editing}

function openPanel(entryId, prefill){
  var existing = null;
  ...
  draft = existing
    ...
    : { id:uid(), name:(prefill&&prefill.name)||"", type:(prefill&&prefill.type)||(db.lastType||"push"),
        sets:[], reps:8, weight:(db.units==="lb"?95:40), editing:false, editingSetIndex:null };
  if(prefill && !existing) prefillFromLast();
  paintPanel();
}
...
function paintPanel(){
  ...
  var recents = suggestNamesByType(draft.type).slice(0,8);
...
function openLibPanel(){ libType = db.lastType||"push"; libEdit=null; paintLibPanel(); }
function closeLibPanel(){ document.getElementById("modal").innerHTML=""; render(); }

function paintLibPanel(){
  var items = (db.exerciseLib[libType]||[]).slice().sort(function(a,b){ return a.localeCompare(b); });

  var h = '<div class="panel" role="dialog" aria-label="Exercise library"><div class="panel-head">'+
    '<button class="ghostbtn" id="cancelLib">Done</button>'+
    '<span class="t">Exercise library</span></div><div class="panel-body">';

  h += '<div class="chips" style="margin-bottom:16px">'+TYPES.map(function(t){
    return '<button class="chip" data-libtype="'+t.key+'"'+(libType===t.key?' style="border-color:'+typeColor(t.key)+';color:var(--bone)"':'')+'>'+esc(t.label)+'</button>';
  }).join("")+'</div>';

  if(!items.length){
    h += '<div style="padding:10px 2px;color:#5b616b;font-size:13px">No exercises saved under '+esc(typeLabel(libType))+' yet.</div>';
  }
  items.forEach(function(n){
    if(libEdit===n){
      h += '<div class="librow editing"><input type="text" id="libEditIn" value="'+esc(n)+'">'+
           '<button class="edbtn" id="libSave" aria-label="Save name">✓</button>'+
           '<button class="rm" id="libCancelEdit" aria-label="Cancel rename">×</button></div>';
    } else {
      h += '<div class="librow"><span class="nm">'+esc(n)+'</span>'+
           '<button class="edbtn" data-libren="'+esc(n)+'" aria-label="Rename '+esc(n)+'">✎</button>'+
           '<button class="rm" data-libdel="'+esc(n)+'" aria-label="Delete '+esc(n)+'">×</button></div>';
    }
  });

  h += '<div class="addrow" style="margin-top:14px">'+
       '<input type="text" id="libNew" autocapitalize="words" autocomplete="off" placeholder="Add exercise to '+esc(typeLabel(libType))+'">'+
       '<button class="ghostbtn" id="libAdd">Add</button></div>';
  h += '</div></div>';
...
function openPlanPanel(planId){
  ...
  planDraft = existing
    ? { id:existing.id, name:existing.name, items:existing.items.map(function(it){ return {name:it.name,type:it.type,sets:it.sets}; }), editing:true }
    : { id:uid(), name:"", items:[], editing:false };
  piType = db.lastType||"push"; piName=""; piSets=3;
  paintPlanPanel();
}
function closePlanPanel(){ planDraft=null; document.getElementById("modal").innerHTML=""; render(); }

function paintPlanPanel(){
  var h = '<div class="panel" role="dialog" aria-label="Workout plan"><div class="panel-head">'+
    '<button class="ghostbtn" id="cancelPlanP">Cancel</button>'+
    '<span class="t">'+(planDraft.editing?"Edit":"New")+' plan</span>'+
    (planDraft.editing?'<button class="ghostbtn" id="delPlan" style="color:var(--plate-red)">Delete</button>':'')+
    '</div><div class="panel-body">';

  h += '<label class="fl" for="planName">Plan name</label>'+
       '<input type="text" id="planName" autocapitalize="words" autocomplete="off" placeholder="Upper body A" value="'+esc(planDraft.name)+'">';

  h += '<div class="queued" style="margin-top:20px"><div class="qhead"><span>Exercises in this plan</span><span>'+
       (planDraft.items.length? planDraft.items.reduce(function(t,i){ return t+i.sets; },0)+" sets" : "—")+'</span></div>';
  if(!planDraft.items.length){
    h += '<div style="padding:18px 2px;color:#5b616b;font-size:13px">No exercises yet. Add one below.</div>';
  }
  planDraft.items.forEach(function(it,i){
    h += '<div class="qrow"><span class="tbadge" style="color:'+typeColor(it.type)+'">'+esc(typeLabel(it.type))+'</span>'+
         '<span class="figure" style="font-size:16px;flex:1">'+esc(it.name)+'</span>'+
         '<span class="setvol">×'+it.sets+'</span>'+
         '<button class="rm" data-rmitem="'+i+'" aria-label="Remove '+esc(it.name)+'">×</button></div>';
  });
  h += '</div>';

  h += '<label class="fl" style="margin-top:20px">Add exercise</label>'+
       '<div class="chips" style="margin-bottom:12px">'+TYPES.map(function(t){
         return '<button class="chip" data-pitype="'+t.key+'"'+(piType===t.key?' style="border-color:'+typeColor(t.key)+';color:var(--bone)"':'')+'>'+esc(t.label)+'</button>';
       }).join("")+'</div>';
  h += '<input type="text" id="piName" list="exlist" autocapitalize="words" autocomplete="off" placeholder="Exercise name" value="'+esc(piName)+'">';
  var sug = suggestNamesByType(piType).slice(0,8);
  if(!piName && sug.length){
    h += '<div class="chips">'+sug.map(function(n){ return '<button class="chip" data-pipick="'+esc(n)+'">'+esc(n)+'</button>'; }).join("")+'</div>';
  }
  h += '<div class="stepper" style="margin-top:12px"><div class="cap">Sets</div><div class="row">'+
       '<button class="pm" data-piadj="-1" aria-label="Fewer sets">−</button>'+
       '<input type="text" inputmode="numeric" id="piSetsIn" value="'+piSets+'">'+
       '<button class="pm" data-piadj="1" aria-label="More sets">+</button></div></div>';
  h += '<button class="primary" id="addItem" style="margin-top:14px">+ Add to plan</button>';

  h += '</div><div class="panel-foot"><button class="primary" id="savePlan"'+
       (planDraft.name.trim()&&planDraft.items.length?"":" disabled")+'>Save plan</button></div></div>';

  document.getElementById("modal").innerHTML = h;
}

function readPlanInputs(){
  var n = document.getElementById("planName");
  if(n) planDraft.name = n.value;
  var pn = document.getElementById("piName");
  if(pn) piName = pn.value;
  var ps = document.getElementById("piSetsIn");
  if(ps) piSets = Math.max(1, Math.min(50, parseInt(ps.value,10)||1));
}
...
document.addEventListener("click", function(ev){
  ...
  /* plans tab */
  if(t.id==="manageLib"){ openLibPanel(); return; }
  if(t.id==="newPlan"){ openPlanPanel(null); return; }
  if(t.dataset.editplan){ openPlanPanel(t.dataset.editplan); return; }
  if(t.dataset.startplan){
    db.runningPlanId = t.dataset.startplan; save();
    tab="log"; cursor=todayKey(); openExercise=null;
    [].forEach.call(document.querySelectorAll("nav.tabs button"), function(b){
      b.setAttribute("aria-selected", b.dataset.tab==="log" ? "true":"false"); });
    render(); return;
  }
  if(t.dataset.endplan){ db.runningPlanId=null; save(); render(); return; }
  if(t.dataset.planitem!==undefined){
    var runPlan2 = db.runningPlanId ? planById(db.runningPlanId) : null;
    if(runPlan2){
      var pitem = runPlan2.items[+t.dataset.planitem];
      var pexisting = dayEntries(cursor).filter(function(e){ return e.name.toLowerCase()===pitem.name.toLowerCase(); })[0];
      openPanel(pexisting?pexisting.id:null, pexisting?null:{name:pitem.name, type:pitem.type});
    }
    return;
  }

  /* exercise library panel */
  if(t.id==="cancelLib"){ closeLibPanel(); return; }
  if(t.dataset.libtype){ libType=t.dataset.libtype; libEdit=null; paintLibPanel(); return; }
  if(t.dataset.libren!==undefined){ libEdit=t.dataset.libren; paintLibPanel(); return; }
  if(t.dataset.libdel!==undefined){ libDelete(libType, t.dataset.libdel); paintLibPanel(); return; }
  if(t.id==="libSave"){
    var libIn=document.getElementById("libEditIn");
    libRename(libType, libEdit, libIn?libIn.value:"");
    libEdit=null; paintLibPanel(); return;
  }
  if(t.id==="libCancelEdit"){ libEdit=null; paintLibPanel(); return; }
  if(t.id==="libAdd"){
    var libNewIn=document.getElementById("libNew");
    libAdd(libType, libNewIn?libNewIn.value:"");
    paintLibPanel(); return;
  }

  /* plan editor panel */
  if(planDraft){
    if(t.id==="cancelPlanP"){ closePlanPanel(); return; }
    if(t.dataset.pitype){ readPlanInputs(); piType=t.dataset.pitype; paintPlanPanel(); return; }
    if(t.dataset.pipick){ readPlanInputs(); piName=t.dataset.pipick; paintPlanPanel(); return; }
    if(t.dataset.piadj){
      readPlanInputs();
      piSets = Math.max(1, Math.min(50, piSets+parseInt(t.dataset.piadj,10)));
      paintPlanPanel(); return;
    }
    if(t.id==="addItem"){
      readPlanInputs();
      if(!piName.trim()) return;
      planDraft.items.push({name:piName.trim(), type:piType, sets:piSets});
      libAdd(piType, piName.trim());
      piName=""; piSets=3;
      paintPlanPanel(); return;
    }
    if(t.dataset.rmitem!==undefined){
      readPlanInputs();
      planDraft.items.splice(+t.dataset.rmitem,1);
      paintPlanPanel(); return;
    }
    if(t.id==="delPlan"){
      if(confirm("Delete this plan? This cannot be undone.")){
        db.plans = db.plans.filter(function(p){ return p.id!==planDraft.id; });
        if(db.runningPlanId===planDraft.id) db.runningPlanId=null;
        save(); closePlanPanel();
      }
      return;
    }
    if(t.id==="savePlan"){
      readPlanInputs();
      if(!planDraft.name.trim() || !planDraft.items.length) return;
      var plist = db.plans;
      var pidx=-1;
      plist.forEach(function(p,i){ if(p.id===planDraft.id) pidx=i; });
      var prec = {id:planDraft.id, name:planDraft.name.trim(), items:planDraft.items};
      if(pidx>=0) plist[pidx]=prec; else plist.push(prec);
      save(); closePlanPanel(); return;
    }
  }
  ...
  if(t.id==="wipeBtn"){
    if(confirm("Delete every logged session, plan, and saved exercise? This cannot be undone.")){
      db={units:db.units,days:{},exerciseLib:{},plans:[],runningPlanId:null}; save(); render();
    }
    return;
  }
...
document.addEventListener("change", function(ev){
  ...
  if(ev.target.id==="planName" && planDraft){
    // don't repaint the whole panel here: a pending tap on a suggestion chip
    // elsewhere in it would land on a detached element once this blur-triggered
    // change event replaces the panel's DOM out from under it. Just keep the
    // Save button's enabled state in sync directly.
    planDraft.name = ev.target.value;
    var saveBtn = document.getElementById("savePlan");
    if(saveBtn) saveBtn.disabled = !(planDraft.name.trim() && planDraft.items.length);
  }
  if(ev.target.id==="fileIn" && ev.target.files[0]){
    ...
    fr.onload=function(){
      try{
        ...
        db={units:d.units||"lb", days:d.days, exerciseLib:d.exerciseLib||{}, plans:d.plans||[], runningPlanId:d.runningPlanId||null};
        save(); render();
...
document.addEventListener("keydown", function(ev){
  ...
  if(ev.key==="Enter" && planDraft){
    if(ev.target.id==="piName"){ ev.preventDefault(); ev.target.blur(); document.getElementById("addItem").click(); }
    if(ev.target.id==="planName"){ ev.preventDefault(); ev.target.blur(); }
  }
  if(ev.key==="Enter" && ev.target.id==="libNew"){ ev.preventDefault(); document.getElementById("libAdd").click(); }
  if(ev.key==="Enter" && ev.target.id==="libEditIn"){ ev.preventDefault(); document.getElementById("libSave").click(); }
});
...
store.get(KEY).then(function(raw){
  if(raw){
    try{
      ...
      if(d && d.days){
        d.exerciseLib = d.exerciseLib || {};
        d.plans = d.plans || [];
        if(d.runningPlanId===undefined) d.runningPlanId=null;
        db=d;
      }
    }catch(e){}
  }
